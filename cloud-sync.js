...
(function(){
  ...
  (function(){
    ...
    let sessionStartedAt=null,startSnapshot=null;
    ...
    function recordAttempt(word,target,selected,isCorrect){if(!cloud||typeof profile==='undefined'||!profile.__cloudUserId||!word||!target||!selected)return;const w=typeof world==='function'?world().id:'orchard';enqueueAttempt({p_session_id:null,p_word:String(word),p_target_letter:String(target),p_selected_letter:String(selected),p_is_correct:Boolean(isCorrect),p_world:String(w)})}
    async function saveWord(word){if(!cloud||!word||typeof profile==='undefined'||!profile.__cloudUserId)return;try{const worldName=typeof world==='function'?world().id:'orchard';const {data}=await cloud.from('word_progress').select('attempts,correct_attempts,mastered').eq('student_id',profile.__cloudUserId).eq('word',word).maybeSingle();const attempts=Number(data?.attempts||0)+1,correct=Number(data?.correct_attempts||0)+1;await cloud.from('word_progress').upsert({student_id:profile.__cloudUserId,word,world:worldName,attempts,correct_attempts:correct,mastered:true,last_attempt_at:new Date().toISOString()},{onConflict:'student_id,word'})}catch(e){console.warn('[FruitTank Cloud] word sync',e)}}
    async function saveDaily(){if(!cloud||typeof profile==='undefined'||!profile.__cloudUserId)return;try{const d=new Date().toISOString().slice(0,10);await cloud.from('daily_quests').upsert({student_id:profile.__cloudUserId,quest_date:d,words_completed:Number(profile.daily||0),target_words:3,completed:Number(profile.daily||0)>=3},{onConflict:'student_id,quest_date'})}catch(e){console.warn('[FruitTank Cloud] daily sync',e)}}
    async function saveSession(){if(!cloud||typeof profile==='undefined'||!sessionStartedAt)return;try{const started=new Date(sessionStartedAt).toISOString(),ended=new Date().toISOString(),w=typeof world==='function'?world():{id:'orchard'},mode=document.getElementById('difficulty')?.value||'Normal';const {error}=await cloud.rpc('record_my_session',{p_world:w.id,p_mode:mode,p_score:Number(state.score||0),p_words_completed:Number(state.words||0),p_shots:Number(profile.shots||0)-Number(startSnapshot?.shots||0),p_correct_attempts:Number(profile.correct||0)-Number(startSnapshot?.correct||0),p_duration_seconds:Math.max(0,Math.round((Date.now()-sessionStartedAt)/1000)),p_started_at:started,p_ended_at:ended});if(error)throw error;await syncProfile();await saveDaily();await flushAttempts()}catch(e){console.warn('[FruitTank Cloud] session sync',e)}sessionStartedAt=null;startSnapshot=null}
    ...
    const originalStart=startGame;startGame=function(){originalStart();sessionStartedAt=Date.now();startSnapshot={shots:Number(profile.shots||0),correct:Number(profile.correct||0)};syncProfile()};
